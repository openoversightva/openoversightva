...
# also drop any rows with a null department (usually just the department name rows)
...
mudf = mudf.set_index(['dept_id','unit']).rename(columns={'id': 'unit_id'})['unit_id'].drop_duplicates().astype('Int64')
...
with engine.connect() as connection: # also create a "Not Sure" job as a default for each dept
...
with engine.connect() as connection:
    odf = pd.read_sql('''select o.*, a.star_no,
        ...
        a.unit_id
        from officers o 
        left join (select a2.*, min(a2.id) over (partition by a2.officer_id) as minaid from assignments a2) a 
            on a.officer_id = o.id and a.id = minaid
        left join jobs j on j.id = a.job_id and j.department_id = o.department_id
    ''', connection)

modf = odf.loc[:,['department_id','last_name','first_name','middle_initial','star_no','unit_id','job_title','unique_internal_identifier']]
modf.columns = ['dept_id','last','first','mi','badge','unit_id','job_title','unique_internal_identifier']
modf = modf.set_index(['dept_id','last','first','mi','badge','unit_id','job_title'])
df = df.set_index(['dept_id','last','first','mi','badge','unit_id','job_title'])
df = df.join(modf['unique_internal_identifier'], how='left') # it would be nice if we could auto update the sheet with this
df.reset_index(inplace=True)

# if there's duplicate uuids, drop all but the first one
df.loc[df.duplicated(['unique_internal_identifier']), 'unique_internal_identifier'] = np.nan
# fill in a unique id for officers missing a badge# and uuid
df.loc[((df['badge']=='') & (df['unique_internal_identifier'].isna())),'unique_internal_identifier'] = df.loc[((df['badge']=='') & (df['unique_internal_identifier'].isna())),'unique_internal_identifier']\
...
df['hired'] = pd.to_datetime(df['hired'], errors='coerce').dt.strftime('%m/%d/%Y')
...
# these names need to match the field names in commands.py/bulk_add_officers etc
